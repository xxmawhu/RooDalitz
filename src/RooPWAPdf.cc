...
void RooPWAPdf::initialize()
{
    ...
    Double_t fx1,fy1,fz1,ft1,fx2,fy2,fz2,ft2,fx3,fy3,fz3,ft3, weight;
    FILE *fp;
    ...
    {
        if( i >= Nmc) break;
        _mcp1[i][0]=ft1;_mcp1[i][1]=fx1;_mcp1[i][2]=fy1;_mcp1[i][3]=fz1;
        _mcp2[i][0]=ft2;_mcp2[i][1]=fx2;_mcp2[i][2]=fy2;_mcp2[i][3]=fz2;
        _mcp3[i][0]=ft3;_mcp3[i][1]=fx3;_mcp3[i][2]=fy3;_mcp3[i][3]=fz3;
        _weight[i] = weight;
        i++;
    }
    fclose(fp);
...
Int_t RooPWAPdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName)const 
{
...
Double_t RooPWAPdf::analyticalIntegral (Int_t code, const char* rangeName)const
{
    assert (code==1);
    Double_t sum=0;
    // for(Int_t i=0;i<Nmc;i++){
        ...
        RooArgList &params)
{
    ...
    RooRealVar *spin = (RooRealVar*)params.at(0);
    // delete arho;
    // delete aphi;
...
Double_t RooPWAPdf::ConstrFactor(const Double_t &FFa0) const
{
    ...
    for(Int_t index=0; index < nAmps; ++index){
        RooRealVar *aRho = (RooRealVar*)_rhoItr->Next();
        RooRealVar *aPhi = (RooRealVar*)_phiItr->Next();
    ...
    ////cout<<" g "<<g<<endl;
    _others->Reset();
    RooRealVar *aLm = (RooRealVar*)_others->Next();
